<template>
  <div class="flex flex-col h-full">
    <Splitter v-model:percent="limitedPercent" :hide-text="{
      'left-pane': '課程清單',
    }" :hide-when-drag="{
      'left-pane': true,
    }">
      <template #left-pane>
        <div class="flex flex-col w-full" style="background-color: rgba(255, 255, 255, 0.1)">
          <a-table sticky @resizeColumn="handleResizeColumn" :row-selection="rowSelection" :columns="columns"
            :data-source="filteredData" :loading="loading" :scroll="{ x: 'max-content' }" :rowClassName="(_record: BaseCourse) => {
            ...
            <template #title>
              <a-flex vertical class="w-full">
                <a-alert v-show="!loading" :message="'資料最近更新時間: ' + lastUpdateTime" type="info" show-icon closable>
                </a-alert>

                <a-flex gap="5" class="my-2">
                  <a-input-search placeholder="課程名稱或代碼" style="width: 200px; margin-right: auto"
                    v-model:value="filter.tempSearch" @search="filter.search = filter.tempSearch" />
                  <a-select style="width: 200px" placeholder="課程分類" @select="department_select"
                    :options="filter.category.options" v-model:value="filter.category.value" />
                  <a-select style="width: 100px" placeholder="年級" :options="filter.Year.options"
                    v-model:value="filter.Year.value" :disabled="filter.Year.disabled" />
            ...
            <template #bodyCell="{ column, value, record }">
              ...
              <template v-if="column.dataIndex === 'course_name'">
                <a-flex vertical>
                  <a-space>
                    <a :href="'https://course.nuk.edu.tw/QueryCourse/' +
                      record.syllabus_link
                      " target="_tauri" rel="noopener noreferrer">
                      {{ value }}
                    </a>
                    <a-tooltip placement="rightTop" v-if="record.online_number >= record.limit">
                      <template #title>
                        <span>線上:{{ record.online_number }} / 名額:{{
                          record.limit
                        }}
                        </span>
                      </template>
              ...
              <template v-else-if="column.dataIndex === 'credits'">
                <a-flex class="w-full" gap="5">
                  <a-space>
                    ...
                    <a-tag :bordered="false" :color="record.requirement_type === '必' ? 'error' : 'cyan'
                      ">
      ...
      <template #right-pane>
        <div class="h-full overflow-y-overlay flex flex-col items-center" style="scrollbar-gutter: stable both-edges">
          <CourseTable :course-list="selectedCourses" @on-course-click="showCourseModal"
            @on-course-clear="clearSelectdCourse" />
        </div>
      </template>
    </Splitter>
    <a-drawer :title="CheckCourseModalData.course.course_name" placement="right" :closable="true"
      :open="CheckCourseModalData.open" :get-container="true" @close="onCloseCourseModal">
...
const filter = reactive({
  ...
  Year: {
    disabled: false,
    options: ["不限", "一年級", "二年級", "三年級", "四年級", "碩一", "碩二"].map(
